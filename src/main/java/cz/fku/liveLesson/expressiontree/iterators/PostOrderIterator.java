package cz.fku.liveLesson.expressiontree.iterators;

import cz.fku.liveLesson.expressiontree.tree.ExpressionTree;

import java.util.Iterator;
import java.util.Stack;

/**
 * Iterates through an @a Tree in post-order.  Plays the role of the
...
public class PostOrderIterator 
    ...
    public ExpressionTree next() {
        ...
        if (!mStack.isEmpty()) {
            ...
            if(!mStack.isEmpty()
               && mStack.peek().left().getRoot() != temp.getRoot()
               && mStack.peek().right().getRoot() != temp.getRoot()) {
                temp = mStack.peek();
